...
from .decorators import ReaderRequiredMixin
from .models import *
from .forms import *
...
class HomeView(ReaderRequiredMixin, ListView):
    model = Article
...
class ArticleCategoryList(ReaderRequiredMixin, ListView):
    model = Article
    template_name = 'reader/article_category_list.html'
    context_object_name = 'articles'
...
class ReaderProfileView(ReaderRequiredMixin, View):
    def get(self, request, pk):
        user = User.objects.get(pk=pk)
        context = {
                    'user':user,
                }
        return render(request, 'reader/reader_profile.html', context)

class WriterProfileView(ReaderRequiredMixin, View):
    def get(self, request, pk):
        user = User.objects.get(pk=pk)
        ...
        return render(request, 'reader/writer_profile.html', context)

class AdminProfileView(ReaderRequiredMixin, View):
    def get(self, request, pk):
        user = User.objects.get(pk=pk)
        published_articles = Article.objects.filter(writer=user, status='PUBLISH').order_by('-views')
        ...
        return render(request, 'reader/admin_profile.html', context)

class ReaderUpdateView(ReaderRequiredMixin, UpdateView):
    model = User
    form_class = ReaderUpdateForm
    template_name = 'reader/user_update.html' # Redirect after successful update
    ...
    def get_success_url(self):
        ...
        return reverse_lazy('reader_profile', kwargs={'pk': self.object.pk})

class ChangePasswordView(ReaderRequiredMixin, PasswordChangeView):
    template_name = 'reader/change_password.html'
    ...
    def get_success_url(self):
        ...
        return reverse_lazy('reader_profile',  kwargs={'pk': self.request.user.pk})

class ArticleDetailView(ReaderRequiredMixin, View):
    def get(self, request, pk):
        ...
        return render(request, 'reader/article_detail.html', {'article': article, 'form': form, 'number_of_likes':number_of_likes, 'comments':comments, 'total_comments':total_comments, 'article_is_liked':liked, 'recent_posts':recent_posts})
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            return redirect(reverse_lazy('reader_article_detail', kwargs={'pk': article.pk}))
        return render(request, 'reader/article_detail.html', {'article': article, 'form': form})

class ArticleLikeView(ReaderRequiredMixin, View):
    def post(self, request, pk):
        article = get_object_or_404(Article, pk=pk)
...
class CommentDeleteView(ReaderRequiredMixin, DeleteView):
    model = Comment
    template_name = 'reader/comment_delete.html'
    ...
    def get_success_url(self):
        ...
        return reverse_lazy('reader_article_detail', kwargs={'pk': article.pk})

class CommentUpdateView(ReaderRequiredMixin, UpdateView):
    model = Comment
    form_class = CommentForm
    template_name = 'reader/article_detail.html'
    ...
    def get_success_url(self):
        ...
        return reverse_lazy('reader_article_detail', kwargs={'pk': self.object.article.pk})
...
class CommentReplyView(ReaderRequiredMixin, CreateView):
    form_class = CommentForm
    template_name = 'reader/article_detail.html'
